...
3. **局部变量**：局部作用域。向循环内部生命的变量，代码块执行完成后撤销，内存被收回。
...
2. `decltype` 类型推导。是“declare type”的缩写，译为“生命类型”。`decltype` 作用是选择并返回操作数的数据类型。
    ```
    auto var = var1 + var2;
...
> 用法
    - **const 成员变量只能在类内生命，在构造函数初始化列表中初始化**
